...
author: "Mateo López, Aitor Arribas y Pedro José Cánovas"
date: "2023-11-13"
output:
  html_document:
    toc: yes
...
cat('Librerías que vamos a utilizar: MASS, knitr, tidyverse, car, dplyr, kableExtra, tidyr, readr, magrittr, VIM, GGally, igraph, ggplot2, sf, leaflet, webshot2, ggmosaic, corrplot, readxl, stringr, ggpubr')
```
...
## Vulnerabilidad
El primer dataset, [Vulnerabilidad](https://valencia.opendatasoft.com/explore/dataset/vulnerabilidad/table/), contiene información de cada barrio, como la densidad de población, la renta media, o el estado de vulnerabilidad. Esta última variable será de gran interés en el análisis que realizaremos posteriormente.
...
Como se puede ver en el código, se ha transformado la variable `Barrrio` a tipo factor,  para poder graficar y tratar la información de forma adecuada. Este proceso de abrir cada conjunto de datos, analizarlo y tratarlo de manera específica se repetirá para cada dataset. Además, se utilizará la variable Barrio como el enlace a entre todos ellos.

## Precio de compra y alquiler
Estos dos datasets de [compra](https://valencia.opendatasoft.com/explore/dataset/precio-de-compra-en-idealista/table/) y [alquiler](https://valencia.opendatasoft.com/explore/dataset/precio-alquiler-vivienda/table/) nos presentan informaciones similares, que es la media de precios de compra y alquiler en los barrios en los años 2022 y 2010. Ya que son conjuntos muy similares, vamos a adelantarnos al próximo paso y fusionarlos en un único dataset, llamado `precios`.
...
## Recibos IBI
Vamos ahora con el dataset [IBI](https://valencia.opendatasoft.com/explore/dataset/rebuts-ibi-2022/table/), que contiene información de los diferentes recibos del IBI (Impuesto sobre Bienes Inmuebles) entre los años 2021 y 2023. Este conjunto nos va a dará una visión acerca de la actividad del barrio, tanto comercial como cultural, turística, religiosa, industrial, etc. 

Dado que en este proyecto no se realizará ningún análisis temporal, vamos a eliminar los años haciendo la media de las observaciones, obteniendo así la misma cantidad de observaciones y barrios.
...
## Bancos por barrio
Por otro lado, tenemos el conjunto de datos [barrios](https://valencia.opendatasoft.com/explore/dataset/bancs-en-via-publica-bancos-en-via-publica/table/), que contiene información acerca de la ubicación de las entidades bancarias en nuestra ciudad. Debido a que nosotros solo vamos a tratar con barrios y no con direcciones, hemos decidido que lo más interesante de este conjunto es el número de bancos que podemos encontrar en cada barrio. Guardaremos esta información en un nuevo dataset llamado `num_bancos`.
...
Este conjunto contiene el área y la población de los diferentes barrios. Hemos considerado este dataset debido a que las variables de áreas y densidad de población que nos proporcionaba el conjunto vulnerabilidad y barrios contenían una gran proporción de valores NA y además no coincidían con otras búsquedas que hemos realizado por internet. Por ello, hemos considerado este otro que se ajusta mucho mejor
...
Observando las 69 variables con las que cuenta nuestro conjunto, vemos como claramente hay muchas que no necesitamos. Para empezar, contamos con todos los códigos de los barrios, que prácticamente cada dataset de los anteriores contaba con una o más variables de este estilo, lo cual es información que no aporta ningún valor. Vamos a empezar eliminandolas aplicando expresiones regulares, ya que todas cuentan con una cosa en común, que empiezan por "cod":
...
De esta manera, hemos reducido el número de variables a 58, pero aún podemos hacer más. También tenemos otra variable redundante, que son los distritos. Como el distrito de compra es el único que no tiene ningún valor perdido, usaremos ese, y además, lo transformaremos en factor:
...
Cabe destacar que este dataframe no es del todo *tidy* hay dos 4 columnas, que dan información sobre los precios por metro cuadrado de alquiler y compra para el año 2010 y 2022. Sin embargo, hemos decidido dejarlo de esta manera, ya que esta parte no es la más relevante en el análisis, y ponerlo *tidy* impricaría duplicar el número de insstancias.

Por tanto, en el momento de estudiar estas variables, se ha creado una dataframe nuevo a partir de `df`, y este si que se ha hecho *tidy* antes de realizar ningún análisis. Para ello, se ha utilizado la función `pivot_longer()`, y las columnas del dataframe *tydy* han resultado Barrio, Año, Precio, y Estado (alquiler o compra).

# Conjunto de datos para mapas

Debido a que hemos realizado un estudio sobre los barrios y distritos de Valencia, nos ha parecido muy adecuado utilizar diferentes mapas para poder entender los datos de una manera visual. Para ello, se han obtenido dos archivos .geojson de la página web de Ayuntamiento de Valencia. En estos, se encuentra la información necesaria para realizar un mapa interactivo, donde se muestran áreas que delimitan clos barrios y distritos de Valencia.

Una vez cargados los archivos, en vez de unirlos al dataframe principal, se ha decidido extraer las variables de interés de esta, y unirlas a los dataframes obtenidos al cargar los archivos .geojson por sencillez.

Estos mapas no serán interactivos en formato pdf.

# Preguntas a resolver

Claramente este conjunto de datos gira entorno a un objeto, los barrios de Valencia. Además, la variable índice de vulnerabilidad consideramos que tiene muchísimo interés. Por tanto, todo nuestro trabajo va a consistir en encontrar las variables que más influyen en esta vulnerabilidad, y encontrar la manera en la que estas contribuyen.

Hemos tratado de responder esta pregunta primero realizando un estudio de la correlación entre todas las variables numericas de nuestro dataset, así como de las dos variables categoricas del mismo. De esta manera, hemos creando diferentes grupos dentro de las variables (en función de la correlación ente estas). Posteriormente, hemos realizado diferentes gráficas para poder relacionar la compleja relación entre las mismas.
...
Para poder hacer un análisis más exahustivo, primero realizaremos un estudio univariante de algunas de las variables que son de mayor interes para este trabajo. Primero realizamos un estudio de las frecuencias en nuestras variables categoricas Distrito y Indice de Vulnerabilidad.
```{r echo=FALSE}
...
Al contrario de lo que podríamos pensar, en nuestra red de correlaciones no vemos una unión entre el precio de compra del metro cuadrado y el precio de alquiler del metro cuadrado, así que vamos a analizar esta relación y como afectan a la vulnerabilidad. Vemos que la correlación entre el metro cuadrado de compra y de alquiler es alta pero no tanto como podríamos esperar.
```{r echo=FALSE}
...
A continuación, vamos a seguir estudiando la vulnerabilidad de los barrios pero mediante diagramas de barras, el cual nos puede dar una idea más cuantitativa. En el siguiente diagrama, se muestran la cantidad de barrios que tiene cada distrito, coloreados en función de la vulnerabilidad. Se trata del mismo grafico que hemos realizado para mostrar la relación entre la vulnerabilidad y los distritos, pero esta vez en formato de barras.
...
Como muestra el mosaico que ya hemos visto anteriormente, la mayoría de personas en Valencia viven en barrios no Vulnerabes, aunque la cantidad de personas que viven en barrios vulnerables o potencialmente vulnerables es mayor.
...
Por otro lado, existen algunas variables que sí han mostrado diferentes distribuciones en función de la vulnerabilidad. A continuación, se muestran las que nos han parecido de mayor interés. Las siguientes figuras muestran aquellas que nos han parecido esperables. Es decir, no es de extrañar que en los barrios vulnerables la tasa de paro sea mayor, o que el índice global (que mide la capacidad de un barrio en términos de innovación) sea mayor en los barrios no vulnerables. Asimismo, observamos que tanto la población como la densidad de población es mayor en barrios vulnerables. Por otro lado, en esta última, cabe destacar que la diferenciaentre la densidad de población en barrios vulnerables y la de barrios potencialmente vulnerables o no vulnerables.
...
En conclusión, para este análisis hemos tratado de contextualizar las variables más relevantes, observarlas, y juzgar su bondad comparándolas con otros datasets o con nuestro conocimiento. Hemos conseguido estudiar la correlación entre las variables numéricas de una manera cuantitativa y sacar los grupos que mayor correlación tienen entre sí. Asimismo, mediante las gráficas de Box-plot y los diagramas concluimos que las variables que más afectan a este son: El paro del barrio (atur_16_,_64), el índice global (Index_Glob), densidad de población, etc.
