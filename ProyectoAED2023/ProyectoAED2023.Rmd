...
vuln$Indice_Vuln<-factor(vuln$Indice_Vuln,levels = unique(vuln$Indice_Vuln))
...
```{r}
# Cambio el nombre de la variable que representa la vulnerabilidad para que sea más intuitiva
colnames(df)[which(colnames(df) == 'Index_Gl_1')] <- 'Vulnerabilidad'
```
Vemos que la columna `vulnerabilidad` es de tipo carácter, y que además hay valores NA. Lo primero de todo se va a cambiar los valores NA por 'indefinido'.

```{r}
# Arrego los valores de 
df$Vulnerabilidad <- factor(df$Vulnerabilidad, levels = c('No Vulnerable', 'Pot. Vulnerable', 'Vulnerable'))
```
...
datos_geojson <- datos_geojson %>%
  mutate(Vulnerabilidad = df$Vulnerabilidad[which(df$Barrio %in% datos_geojson$nombre)])

popups <- paste0("<b>", datos_geojson$nombre, "</b>", "<hr>", datos_geojson$Vulnerabilidad)

pal <- colorFactor(c('green','gray','blue','red'), levels = levels(datos_geojson$Vulnerabilidad))

leaflet(data = datos_geojson) %>%
  addTiles() %>%
  addPolygons(fillColor = pal(datos_geojson$Vulnerabilidad),
              weight = 1,
              opacity = 1,
