...
Otra que resalta de las variables provenientes del dataset IBI es que cuentan con un mínimo de 0, mientras que la media de los valores ronda valores muy altos. Esto se puede deber a que, en vez de poner un NA en los datos al no saber la información, se ha puesto un 0 en su lugar. Vamos a transformar estos 0 en NA para darles un tratamiento correcto.

```{r}
df%<>%mutate(across(starts_with("Imp"),~ifelse(.==0,NA,.)))
```
